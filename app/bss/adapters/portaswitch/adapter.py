...
from bss.models import DeliveryChannel, UserCreateResponse, CustomRequest, CustomResponse
...
class Adapter(BSSAdapter):
    ...
    def __init__(self, config: AppConfig):
        ...
        self._contacts_selecting_ext_types = [PortaSwitchExtensionType(type) for type in
                                              ext_types] if ext_types else list(
        ...
        self._config_portal_url = config.get_conf_val('PortaSwitch', 'SELF', 'CONFIG', 'PORTAL', 'URL')
    ...
    def retrieve_contacts(self, session: SessionInfo, user: UserInfo) -> list[ContactInfo]:
        ...
        """
        try:
            account_info = self.__account_api.get_account_info(safely_extract_scalar_value(session.access_token))[
                'account_info']
    ...
    def retrieve_voicemails(self, session: SessionInfo, user: UserInfo) -> UserVoicemailsResponse:
        ...
        """
        try:
            mailbox_messages = self.__account_api.get_mailbox_messages(
    ...
    def retrieve_voicemail_message_attachment(self, session: SessionInfo, message_id: str,
                                              file_format: str) -> Iterator:
    ...
    def custom_method_public(self,
        ...
        if method := getattr(self, attr_name, None):
            ...
            raise WebTritErrorException(status_code=404, error_message=f"Method '{method_name}' not found",
    ...
    def _self_config_portal_url(self, user_id: str, data: CustomRequest) -> CustomResponse:
        account_info = self.__admin_api.get_account_info(i_account=user_id).get('account_info')
        session_data = self.__account_api.login(account_info['login'], account_info['password'])

        return CustomResponse(
            url=f"{self._config_portal_url}?token={session_data['access_token']}",
